...
@Component({
  ...
  templateUrl: './ccif-retrait-hist.component.html',
  styleUrls: ['./ccif-retrait.component.css']
})
export class CcifRetraitHistComponent implements OnInit, AfterViewInit {
  imageUrl = apiURL('/download/');
  collecteur: Collecteur[] = [];
  collecte: Collecte[] = [];
  id_cpe: number;
  TOTAL: any;
  displayedColumns: string[] = ['photo', 'ref', 'nom', 'prenom', 'montant', 'dateR'];
  ...
  async getDatas() {
    this._progress = true;
    this._api.getRetraitHist(this._pageEvent.pageIndex, this._pageEvent.pageSize).subscribe((res: any) => {
      this._progress = false;
      console.log(res.body.data);
      this._matPaginator.length = res.body.metadata;
      // this.TOTAL = res.body.totalCollecte;
      this.data = res.body.data;
      this.TOTAL = res.body.data.reduce((acc, current) => acc + current.montantRetrait, 0);
